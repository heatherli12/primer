...
<!-- Highest priority is making sure that what we have is good. No ugliness or embarassments. -->

<!-- Take ownership of the Advanced Plots section. Add the tips and tricks in the comments. -->

<!-- Create "Going further" section. It includes some cool plots, without the code for making them (I think) and with a brief (short paragraph) description of the package which is used to make them and references for learning more. One animation. One map. One interactive. DONE -->

<!-- https://github.com/PPBDS/primer/blob/d11aae84a8562caedc94615b8de28f3620af8bf7/01-visualization.Rmd -->

<!-- Do the census and maps tutorials from chapter 3. Consider https://walker-data.com/census-r/ DONE-->
...
# Data is from 538, this is the data for the first 100 days of both Biden's and Trump's presidency. Changed the columns to date formates, but also bc the term starts on January 20th instead of January 1st I needed to subtract 20 days to make sure January 20 was counted as "Day 1"
...
Before we take a look at the other useful geoms, let's first talk about colour and fill. These are two arguments we can use to change the colour of geoms we have. 
...
ggplot(data = trains, 
  ...
  labs(title = "Age and Income Among Boston Commuters",
       ...
       caption = "Data source: Enos (2014)")
```
You'll notice that the plot is the exact same as the plot in the previous section, with one key exception: we've added colours that separate the commuters by party. How did we do this? You'll notice in the `mapping = aes()` function, we've added another argument. This argument, `color = `, can change the colour of the dots by a certain value. In this case, we changed the colour by party.

You can also change the dots to another colour. For instance, take this plot below:
...
ggplot(data = trains, 
       mapping = aes(x = age, 
                     y = income,
                     color = "steel blue")) + 
  # for some reason this is always this colour regardless of what i change it to, like steel blue isn't blue?
  geom_point() +
  labs(title = "Age and Income Among Boston Commuters",
       subtitle = "Older commuters don't seem to make more money",
       x = "Age",
       ...
       caption = "Data source: Enos (2014)")
```
By setting the color argument equal to `steelblue`, we could change the dots to blue. R has 657 built-in color names, which you can view if you run the command `grDevices::colors()`. Give it a try in your console! If a colour you want isn't in the 657 inbuilt ones, you can also change the colour with a HEX colour code.

#### Difference Between Fill and Colour

There is another argument you can use, called `fill`. The key difference between fill and colour is that fill defines the colour with which a geom is filled, whereas colour moreso defines the outline of a particular geom. For example:
...
ggplot(data = trains,
  ...
  geom_bar(color = "pink")
```
You can see here that `color` has only changed the very outline of the bars. Comparatively, if we used `fill`:
...
You get pink bars instead. You can change the fill and color of all the geoms we show you below, so happy colouring!
...
You can make your plots interactive, using the *plotly* package. Remember, you need to load packages before you use them, so load `library(plotly)` if you want to make your own interactive plots. Check out this version of
