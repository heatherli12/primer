...
<!-- Plot should be much cooler, perhaps even using big data. And an associated map. Precinct level voting changes 2012 -> 2016 -> 2020, perhaps. -->
...
<<<<<<< HEAD
<!-- HL: is it possible to fix the graph? the subtitle overlapping with the title isn't great aesthetically and also probably isn't the best "hey look pretty graphs" appeal -->
=======
...
>>>>>>> 11659bfea2c49f4b18d6eeb53e9f7fad193dfad5
...
<!-- HL: we could probably just skip the whole "this isn't like Excel" and just say "we'll be teaching you the coding language R, it's a language like python or javascript" and then move on -->

<!-- Skip the whole "this isn't like Excel" and just say "we'll be teaching you the coding language R, it's a language like python or javascript" and then move on -->
...
* *Data types*: integers, doubles/numerics, logicals, and characters. Integers are values like -1, 0, 2, 4092. Doubles or numerics are a larger set of values containing both the integers but also fractions and decimal values like -24.932 and 0.8. Logicals are either `TRUE` or `FALSE`. Characters are text like "cabbage", "Hamilton", or "The Wire is the greatest TV show ever." Characters are often denoted with the quotation marks around them.

<<<<<<< HEAD
<!-- HL: could we maybe separate this into individual bullet points explaining each one? it looks rather crowded just shoved under the "data types" header -->
=======
<!-- Separate this into individual bullet points explaining each one. it looks rather crowded just shoved under the "data types" header -->
>>>>>>> 11659bfea2c49f4b18d6eeb53e9f7fad193dfad5
...
<<<<<<< HEAD
<!-- HL: i think we could probably explain factors and categorical data more, even after reading wrangling I'm still somewhat unsure what it means and it might be good to have a more in-depth explanation here for those of us who aren't super familiar with data terms -->
...
<!-- HL: I think it's somewhat unclear what "modern data frame" means, maybe elaborate? -->
=======
...
<<<<<<< HEAD
<!-- HL: can we explain the difference between categorical and quantitative variables? -->
=======
...
The problem with this display is "overplotting." Because attitudes are measured as integers, we do not not know if a given point represents just one person or a dozen. There are two methods we can use to address overplotting: transparency and jitter.

<<<<<<< HEAD
<!-- HL: provide a more concrete definition of what overplotting is + why that's bad for visual representation of data? -->
=======
>>>>>>> 11659bfea2c49f4b18d6eeb53e9f7fad193dfad5
...
<!-- HL: maybe add a concrete definition of what jittering is? it's sort of implied in the paragraph below but maybe defining it specifically would be good -->
...
Linegraphs show the relationship between two numerical variables when the variable on the x-axis, also called the *explanatory* or *predictive* or *independent* variable, is of a sequential nature. In other words, there is an inherent ordering to the variable. 


<!-- HL: maybe it's just a Canadian thing, but I think a decent amount of high schoolers are taught that it's called the dependent/independent variable? would that be good to include so hs students can relate to terms they learned in stat class? or is this something different -->
...
<<<<<<< HEAD
<!-- HL: I feel like this stuff below about you not needing to specify the arguments is sort of randomly dropped in the middle here, maybe this could be added to an "additional info" section at the bottom? isn't really crucial to the lesson after all -->
=======
...
<<<<<<< HEAD
<!-- HL: give an example of the binwidth argument and how to use it? -->
=======
<!-- HL: give an example of the binwidth argument and how to use it? Maybe? -->
>>>>>>> 11659bfea2c49f4b18d6eeb53e9f7fad193dfad5
...
<<<<<<< HEAD
<!-- HL: could we elaborate on geom_col() more? give it its own mini section bc I think it's distinct enough and people might not understand why you need to use geom_col if it's so similar to geom_bar -->
=======
<!-- HL: could we elaborate on geom_col() more? give it its own mini section bc I think it's distinct enough and people might not understand why you need to use geom_col if it's so similar to geom_bar. Add a code chunk with does group_by and summarize and then pipes to geom_col. -->
>>>>>>> 11659bfea2c49f4b18d6eeb53e9f7fad193dfad5
...
Another use of barplots is to visualize the *joint* distribution of two categorical variables. Let's look at `race`, as well as `treatment`, in the `trains` data by using the `fill` argument inside the `aes()` aesthetic mapping. Recall the `fill` aesthetic corresponds to the color used to fill the bars.

<!-- HL: define what joint distribution means? -->
...
The pipe operator (`%>%`) allows us to combine multiple operations in R into a single sequential *chain* of actions. Much like how the `+` sign has to come at the end of the line when constructing plots --- because we are building the plot layer-by-layer --- the pipe operator `%>%` has to come at the end of the line because we are building a data wrangling pipeline step-by-step.

<<<<<<< HEAD
<!-- HL: could maybe include smth like "The pipe operator tells R to go to the next step. If you don't include the pipe operator, R assumes the next command has nothing to do with the changes you made before and you'll get an error"  -->
=======
<!-- Include something like "The pipe operator tells R to go to the next step. If you don't include the pipe operator, R assumes the next command has nothing to do with the changes you made before and you'll get an error"  -->
>>>>>>> 11659bfea2c49f4b18d6eeb53e9f7fad193dfad5
...
Instead of create a single criteria with many parts, liked with an `&`, you can just separate the parts with a comma. The resulting tibble is the same.
...
Whenever we create a new tibble, or a new variable within a tibble, we have a choice: Should we overwrite the existing tibble/variable or create a new one? There is no right answer. 
